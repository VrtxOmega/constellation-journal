...
// ═══════════════════════════════════════════════════════════
// IN-APP SCREEN RECORDER & DEMO MACRO (Phase 13)
// ═══════════════════════════════════════════════════════════
...
function initRecorder() {
  document.addEventListener('keydown', async (e) => {
    // Ctrl+Shift+R to manually toggle recording
    if ((e.ctrlKey || e.metaKey) && e.shiftKey && e.key.toLowerCase() === 'r') {
      e.preventDefault();
      if (isRecording) stopRecording();
      else startRecording();
    }
    // Ctrl+Shift+D to run perfect demo
  ...
  // Add flashing dot keyframes
  const style = document.createElement('style');
  style.textContent = `
    @keyframes pulse-record {
      0% { opacity: 1; box-shadow: 0 0 10px red; }
      100% { opacity: 0.3; box-shadow: 0 0 2px red; }
    }
  `;
...
async function startRecording() {
  try {
    const sourceId = await window.journal.getDesktopSourceId();
    if (!sourceId) return console.error('No window source found');
    ...
    mediaRecorder.onstop = async () => {
      ...
      const savePath = await window.journal.saveRecording(buffer);
      isRecording = false;
      document.getElementById('recording-indicator')?.remove();
      
      const toast = document.createElement('div');
      toast.innerHTML = `🎥 Demo saved to <b>Desktop/constellation-demo.webm</b>`;
      toast.style.cssText = 'position:fixed;top:30px;left:50%;transform:translateX(-50%);background:rgba(212,175,55,0.15);border:1px solid #d4af37;color:#d4af37;padding:12px 24px;border-radius:20px;z-index:9999;font-family:Philosopher,serif;backdrop-filter:blur(4px);';
    ...
    ind.style.cssText = 'position:fixed;top:40px;right:40px;width:12px;height:12px;background:red;border-radius:50%;z-index:9999;animation:pulse-record 0.8s infinite alternate;';
...
function typeSimulated(text, durationMs, callback) {
  ...
  const iv = setInterval(() => {
    ta.value += text[i++];
    if (document.getElementById('typewriter-checkbox') && document.getElementById('typewriter-checkbox').checked) {
       playTypewriterClick();
    }
    updateWordCount();
    if (i >= text.length) {
...
function runDemoSequence() {
  ...
  // 5. Epic Pan Shot across the real stars
  setTimeout(() => {
  ...
  // 6. Cut
  setTimeout(() => { stopRecording(); window._autoRotate = true; }, 56000);
}
